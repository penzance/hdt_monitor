...
exec {'drop-existing-project-user':
    ...
    command => 'psql -d postgres -c "DROP USER IF EXISTS hdt_monitor"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-user':
    ...
    command => 'psql -d postgres -c "CREATE USER hdt_monitor WITH PASSWORD \'hdt_monitor\'"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'drop-project-db':
    ...
    command => 'psql -d postgres -c "DROP DATABASE IF EXISTS hdt_monitor"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
exec {'create-project-db':
    ...
    command => 'psql -d postgres -c "CREATE DATABASE hdt_monitor WITH OWNER hdt_monitor"',
    user => 'postgres',
    group => 'postgres',
    logoutput => true,
}
...
# Create a symlink from ~/hdt_monitor to /vagrant as a convenience for the developer
file {'/home/vagrant/hdt_monitor':
    ensure => link,
    target => '/vagrant',
}

# Create a virtualenv for hdt_monitor
exec {'create-virtualenv':
    provider => 'shell',
    user => 'vagrant',
    group => 'vagrant',
    require => [ Package['virtualenvwrapper'], File['/home/vagrant/hdt_monitor'], Exec['known_hosts'], Exec['drop-project-db']],
    environment => ["HOME=/home/vagrant","WORKON_HOME=/home/vagrant/.virtualenvs"],
    command => '/vagrant/vagrant/venv_bootstrap.sh',
    creates => '/home/vagrant/.virtualenvs/hdt_monitor',
}

# set the DJANGO_SETTINGS_MODULE environment variable
file_line {'add DJANGO_SETTINGS_MODULE env to postactivate':
    ensure => present,
    line => 'export DJANGO_SETTINGS_MODULE=hdt_monitor.settings.local',
    path => '/home/vagrant/.virtualenvs/hdt_monitor/bin/postactivate',
...
file_line {'add DJANGO_SETTINGS_MODULE env to postdeactivate':
    ...
    path => '/home/vagrant/.virtualenvs/hdt_monitor/bin/postdeactivate',
    require => Exec['create-virtualenv'],
}
